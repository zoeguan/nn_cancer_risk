...
for (i in prop.missing) {
  print(i)
  nn.input.i = nn.input[ind.test, ]
  for (rel.type in rel.names) {
    ind.aff = which(nn.input[ind.test, paste0("AffB_", rel.type)]==1)
    ...
    ind.aff.ov = which(nn.input[ind.test, paste0("AffO_", rel.type)]==1)
...
for (i in prop.missing) {
  ...
  nn.input.i = nn.input[ind.test, ]
  for (rel.type in rel.names) {
    ind.aff = which(nn.input[ind.test, paste0("Miss_", rel.type)]==0)
    ind.sample = sample(ind.aff, round(i*length(ind.aff)), replace=F)
...
for (i in prop.missing) {
  ...
  nn.input.i = nn.input[ind.test, ]
  for (rel.type in rel.names) {
    ind.aff = which(nn.input[ind.test, paste0("AffB_", rel.type)]==0 & nn.input[ind.test, paste0("AffO_", rel.type)]==0)
    ind.sample = sample(ind.aff, round(i*length(ind.aff)), replace=F)
    nn.input.i[ind.sample, paste0(c("AffB_", "AgeB_", "AffO_", "AgeO_", "Age_"), rel.type)] = 0
    nn.input.i[ind.sample, paste0(c("Miss_"), rel.type)] = 1
  }
