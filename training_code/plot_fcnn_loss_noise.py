...
pred_fcnn = fcnn.predict(X_train[720000:len(y_train)]).flatten()
perf_boot = boot(pred_fcnn, y_train[720000:len(y_train)])

results ={'auc': perf_boot[0:3],
         'oe': perf_boot[3:6],
         'bs': perf_boot[6:9]}
...
figname = "../tuning_results/val_loss_noise_" + str(max_train) + "_" + str(epochs)
fig = plt.figure()
...
figname = "../tuning_results/val_auc_noise_" + str(max_train) + "_" + str(epochs)
fig2 = plt.figure()
plt.plot(history.history['val_soft_AUC_theano'])
fig2.savefig(figname)
plt.close(fig2)

figname = "../tuning_results/val_oe_noise_" + str(max_train) + "_" + str(epochs)
fig3 = plt.figure()
