...
for i in range(len(suffix_list)):
	...
	nn_test = pd.read_csv('../nn_input/cnntest_input_' + suffix + '.csv', low_memory=False)
	

	X_test_cnn = np.array(scaler2.transform(nn_test.iloc[0:test_size, start_x:(end_x)]))
	...
	filename = "../results/res_cnntest_input_" + suffix + "_" + str(max_train) + "_" + str(epochs) + "_" + str(seed_value) + ".csv"
