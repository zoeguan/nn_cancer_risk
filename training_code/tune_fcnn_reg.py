...
reg_vals = [0.0001, 0.001, 0.01, 0.1]
...
for i in range(len(reg_vals)):
    print(i)
    fcnn = Sequential()
    fcnn.add(Dense(30, input_shape=(X_train.shape[1],), activation='elu', kernel_regularizer=l2(reg_vals[i])))
    fcnn.add(Dense(10, activation='elu', kernel_regularizer=l2(reg_vals[i])) )
    fcnn.add(Dense(1, activation='sigmoid', kernel_regularizer=l2(reg_vals[i])) )
...
perf_df.to_csv('../tuning_results/tune_fcnn_reg.csv')
