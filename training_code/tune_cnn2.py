...
for i in range(len(size1)):
    print(i)
    g_model = Sequential()
    g_model.add(GraphConv(filters=size1[i], neighbors_ix_mat = graph_mat, num_neighbors=num_neighbors, activation='elu', input_shape=(X_train_2.shape[1], features)))
    g_model.add(Dropout(0.2))
    g_model.add(GraphConv(filters=size2[i], neighbors_ix_mat = graph_mat, num_neighbors=num_neighbors, activation='elu'))
...
perf_df.to_csv('../tuning_results/tune_cnn2.csv')
